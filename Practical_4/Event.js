...
// In-memory fallback (will be replaced by MongoDB)
...
async function connectToMongoDB() {
  try {
    ...
    db = client.db(); // Use the database from connection string
...
// Event listeners with MongoDB integration
userEvents.on('user-register', async (username) => {
  try {
    ...
    console.error('Error storing register event:', error);
  }
});

userEvents.on('user-login', async (username) => {
  try {
...
userEvents.on('user-logout', async (username) => {
  try {
    ...
    console.error('Error storing logout event:', error);
  }
});

userEvents.on('user-purchase', async (username, productName, quantity, price) => {
  try {
...
userEvents.on('profile-update', async (username) => {
  try {
    ...
    console.error('Error storing profile update event:', error);
  }
});
...
const server = http.createServer((req, res) => {
  ...
  if (pathname === '/' && req.method === 'GET') {
    ...
    // Handle profile update with MongoDB
    let body = '';
    req.on('data', chunk => {
      body += chunk.toString();
    });
    req.on('end', async () => {
      ...
      try {
        ...
        res.setHeader('Content-Type', 'application/json');
        res.statusCode = 500;
        res.end(JSON.stringify({ success: false, message: "Profile update failed!" }));
      }
    });
  }
    ...
    // Serve dashboard with MongoDB data
    (async () => {
      try {
        ...
        const eventCountsFromDB = await eventsCollection.aggregate(pipeline).toArray();
        
        // Convert to expected format
        const dbEventCounts = {};
        eventCountsFromDB.forEach(item => {
          dbEventCounts[item._id] = item.count;
        });
        
        // Get all events from MongoDB
        const eventsFromDB = await eventsCollection.find().sort({ timestamp: -1 }).toArray();
        ...
        const usersFromDB = await usersCollection.find({}, { projection: { username: 1, _id: 0 } }).toArray();
        
        res.setHeader('Content-Type', 'application/json');
        res.statusCode = 200;
        res.end(JSON.stringify({ 
          eventCounts: dbEventCounts, 
          eventLogs: formattedEvents,
          registeredUsers: usersFromDB
        }));
      } catch (error) {
